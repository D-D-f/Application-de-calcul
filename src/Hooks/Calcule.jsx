...
const UseCalcule = () => {
  ...
  const convertTime = (data) => {
    ...
    let bn = data?.beforetime
      ? beforeBaseNavale[data?.base_navale]
      : baseNavale[data?.base_navale];

    const timeSeconds = Math.ceil(
      hours * 3600 + minutes * 60 + seconds + bn * Number(data?.distance)
    );

    const timeSecondsWithPo = Math.ceil(
      hours * 3600 + minutes * 60 + seconds + (bn / 2) * Number(data?.distance)
    );
